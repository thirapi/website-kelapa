...
- **Responsif** (mobile-first): H1 `text-4xl sm:text-5xl md:text-7xl` · H2 section
  ...
  teks di atas gelap selalu token `cream` (bukan `white`), panel terang `surface`.
- **Jarak awal section**: default pakai ritme `Section` (py-24/md:py-32).
  Section pertama setelah `PageHeader` yang berisi filter/konten langsung:
  `pt-10 md:pt-14` (PageHeader sudah punya py + border sendiri).
  Jangan tuning manual per halaman di luar dua pola ini.
- **Filter pills**: satu baris scroll horizontal, kriteria `FilterPills`
  (`ui/FilterPills.tsx`). Dilarang bikin varian pill sendiri.
