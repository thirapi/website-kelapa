...
- Bahasa: **English** (isi `referensi.md` berbahasa Inggris; pasar B2B global).

## 2. Produk (katalog kini + depan)

Kini: Copra · Coconut Shell Charcoal · Briquettes. Depan: Coconut Oil, VCO
(`Future Products`). Detail spec per `referensi.md` (moisture, ash, fixed
carbon, size, packing, origin).

## 3. Persona & KPI

- **Buyer B2B**: butuh foto produk, spec, MOQ, availability, harga/request-price,
  RFQ gabungan (product + quantity + spec + packaging + destination + delivery date).
- KPI: RFQ submit ≥ 1,5% sesi · klik WhatsApp ≥ 3% · scroll depth ≥ 70% ·
  LCP < 2,5s · CLS < 0,1 · nol klaim tanpa sumber.

## 4. Open Questions (pemilik data)
...
- [ ] Logo Pertamina & DEB (file + aturan pakai) untuk logo wall.
- [ ] Endpoint form (email/webhook/CRM). Backend Exantara = keputusan eksternal.
